...
EOF
    ...
    [[ $status == 0 ]] && echo && echo $(date) cookies登录${dbg_username:0:2}******${dbg_username:8}成功
    ...
    if [[ $status == 1 ]]; then
        ...
        [[ "$token" = "" ]] && echo && echo $(date) ${dbg_username:0:2}******${dbg_username:8} Login Failed. && rm -rf $workdir && return 1
        echo && echo $(date) 密码登录${dbg_username:0:2}******${dbg_username:8}成功
    fi
}
...
function jifeninfo() {
    ...
    # info
    echo $(echo ${dbg_username:0:2}******${dbg_username:8}) 总积分-$canUse 通信积分-${jfnumber[0]} 奖励积分-${jfnumber[1]} 定向积分-${jfnumber[2]} 本月将过期积分:$invalid 本月将过期奖励积分:$invalidscore 本月新增奖励积分:$addScore 本月消耗奖励积分:$decrScore 昨日奖励积分:$yesterdayscore 今日奖励积分:$todayscore
}
...
function formatsendinfo() {
    ...
    if $(echo ${all_parameter[*]} | grep -qE "sendsimple"); then
        ...
        echo ${all_parameter[*]} | grep -qE "jifeninfo" && echo $(echo ${dbg_username:0:2}******${dbg_username:8}) 总积分-$canUse 通信积分-${jfnumber[0]} 奖励积分-${jfnumber[1]} 定向积分-${jfnumber[2]} 本月将过期积分:$invalid 本月将过期奖励积分:$invalidscore 本月新增奖励积分:$addScore 本月消耗奖励积分:$decrScore 昨日奖励积分:$yesterdayscore 今日奖励积分:$todayscore >>$formatsendinfo_file
...
function main() {
    for ((u = 0; u < ${#all_username_password[*]}; u++)); do 
        ...
        userlogin && userlogin_ook[u]=$(echo ${dbg_username:0:2}******${dbg_username:8}) || { userlogin_err[u]=$(echo ${dbg_username:0:2}******${dbg_username:8}); continue; }
        membercenter
        liulactive
        hfgoactive
        jifeninfo
